    ...
    <section className="relative w-full overflow-hidden bg-gradient-to-br from-foreground to-foreground/95 text-background py-16 sm:py-20 lg:py-24 xl:py-32">
      ...
      <div className="relative w-full px-4 sm:px-6 lg:px-12 xl:px-20 max-w-[1800px] mx-auto">
        <div className="text-center max-w-4xl mx-auto mb-16">
          ...
          <p className="text-base sm:text-lg md:text-xl text-background/70 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
          {/* PRIMA */}
          <div className="rounded-2xl border border-destructive/30 bg-destructive/5 backdrop-blur-sm p-6 sm:p-8 space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-destructive/20 border border-destructive/30 rounded-full">
              <XCircle className="w-4 h-4 text-red-400" />
              <span className="text-sm font-bold text-red-400">PRIMA</span>
            </div>
            <div className="space-y-4">
              {PROBLEMS.map((item) => (
                <div key={item.title} className="flex gap-4 p-4 rounded-xl bg-background/5 border border-background/10">
                  <div className="flex-shrink-0 w-10 h-10 bg-destructive/20 rounded-xl flex items-center justify-center">
                    <item.icon className="w-5 h-5 text-red-400" />
                  </div>
                  <div>
                    <h3 className="font-bold text-background mb-1">{item.title}</h3>
          ...
          {/* DOPO */}
          <div className="rounded-2xl border border-primary/30 bg-primary/5 backdrop-blur-sm p-6 sm:p-8 space-y-6">
            ...
            <div className="space-y-4">
              {SOLUTIONS.map((item) => (
                <div key={item.title} className="flex gap-4 p-4 rounded-xl bg-background/5 border border-primary/10">
        ...
        <div className="mt-12 bg-primary/20 backdrop-blur-sm border-2 border-primary/30 rounded-3xl p-8 text-center">
          <p className="text-xl sm:text-2xl font-bold text-background mb-2">
            Il chatbot AI converte visitatori in lead qualificati
          </p>
          <p className="text-base sm:text-lg text-background/80">
